"""
v0.5: Safety response: Mission abort

New behaviors:
- Clamp remaining capacity so it never goes negative
- Abort/Derate mission when safety triggers happen:
    - If overcurrent -> immediately enter abort mode (thrusters go to 0 or safe idle)
    - If If low battery -> enter abort mode
...
def run_sim():
    ...
    with log_path.open("w", newline="") as f:
	    ...
	    if aborted:
	    	# Brownout-safe mode: kill thrusters, keep compute + sensors alive
    		if "thrusters" in per_load:
        	    per_load["thrusters"] = 0.0
		...
		remaining_ah = max(0.0, remaining_ah)
		...
		# Decide abort condition (latch on first trigger)
		if (overcurrent_warn or low_capacity_warn) and not aborted:
   		    aborted = True
    		    abort_reason = "overcurrent" if overcurrent_warn else "low_battery"
    		    print(f"🛑 ABORT: triggered at t={total_time_s}s due to {abort_reason}")
